function [overall_mssim] = ms_ssim_index_sal(img1, img2, sal, K, win, level, weight, method)
...
function [mssim, ssim_map, mcs, cs_map] = ssim_index_new(img1, img2, sal, K, win)
...
% mcs = mean2(cs_map);
sal = imresize(sal, [size(ssim_map,1) size(ssim_map,2)]);
mssim = sum(sum(ssim_map.*sal)) / sum(sum(sal));
mcs = sum(sum(cs_map.*sal)) / sum(sum(sal));
